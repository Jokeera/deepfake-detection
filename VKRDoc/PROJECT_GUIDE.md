...
Все 4 эксперимента ablation study запускаются через Kaggle notebooks:
- `kaggle-train.ipynb` — single-dataset (DFDC02)
- `kaggle-multi-train.ipynb` — multi-dataset (DFDC02+DFD01)
- `kaggle-train-3ds-t16.ipynb` — 3-dataset (DFDC02+DFD01+CelebDF)
...
> **Вывод**: добавление CelebDF повысило AUC с 0.899 до 0.959 (+6.0%) за счёт увеличения объёма и разнообразия обучающей выборки. Full dual-path модель — лучшая на 3 датасетах.

### Эксперимент 5: 3-Dataset T=32 (увеличенное число кадров)

Увеличение числа кадров с 16 до 32 (batch_size=8). 3 датасета, ~9953 видео.
...
> **Вывод**: T=32 значительно улучшает все метрики. Full model: AUC 0.9587→0.9943 (+3.6%), EER 0.1211→0.0406 (−66%). Ablation study подтверждён: dual-path архитектура лидирует при любом T.
...
A: Cross-dataset показал, что модель плохо обобщает (AUC 0.55). Multi-dataset training учит модель на разных методах генерации. Обучение на 3 датасетах (DFDC02+DFD01+CelebDF) дало AUC 0.959 — модель стала значительно более робастной.
...
6. `VKRDoc/VKR_FINAL(v14).docx` — текст диссертации
...
# Проверка структуры данных: должны быть подпапки real/ и fake/ с JPEG кадрами
ls data/preprocessed_data/preprocessed_DFDC02_16/
